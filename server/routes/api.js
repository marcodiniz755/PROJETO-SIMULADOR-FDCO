...
router.post('/calculate-fam', (req, res) => {
    try {
        const { ipca_m1, ipca_m2, ndup, nudsa, ndus, ndupx, ndmp, ndms } = req.body;
        
        // Validação dos parâmetros
        const params = [ipca_m1, ipca_m2, ndup, nudsa, ndus, ndupx, ndmp, ndms];
        ...
        // Cálculo do FAM usando fórmula correta
        // IPCA vem como 0.24, 0.26 (percentual) - converter para decimal (0.0024, 0.0026)
        const pi_m1 = ipca_m1 / 100;
        const pi_m2 = ipca_m2 / 100;
        
        // FAM = (1 + pi_m2)^(ndup/ndmp) * (1 + pi_m1)^(ndus/ndms)
        // onde: ndmp = nudsa + ndup e ndms = ndupx + ndus
        const exp1 = ndup / ndmp;
        const exp2 = ndus / ndms;
        
        const termo1 = Math.pow(1 + pi_m2, exp1);
        const termo2 = Math.pow(1 + pi_m1, exp2);
        ...
        res.json({
            ...
            parametros: {
                ipca_m1: parseFloat(pi_m1.toFixed(4)),
                ipca_m2: parseFloat(pi_m2.toFixed(4)),
                ndup, nudsa, ndus, ndupx, ndmp, ndms,
                exp1: parseFloat(exp1.toFixed(10)),
                exp2: parseFloat(exp2.toFixed(10)),
